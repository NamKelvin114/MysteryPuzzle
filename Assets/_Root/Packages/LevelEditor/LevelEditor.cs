...
namespace Dacodelaac.LevelSystem
{
    ...
    {
        string AssetPath => "Assets/Roots/Prefabs";
        string FolderPath => Path.Combine(Application.dataPath, "Roots/Prefabs/Element");
        ...
        void OnGUI()
        {
            ...
            {
                var levelAsset =
                    AssetDatabase.LoadAssetAtPath<LevelMap>(PrefabStageUtility.GetCurrentPrefabStage().assetPath);
                if (levelAsset)
                {
                    GUI.enabled = false;
        ...
        void DrawPrefabSections()
        {
            ...
            if (!string.IsNullOrEmpty(FolderPath))
            {
                ...
                if (directoryInfo.Exists)
                {
                    ...
                    {
                        //if (new string[]{"Level", "Popup", "UI","JigsawLevels","HardLevels"}.All(p => directory.FullName.IndexOf(p) == -1)) 

...
#pragma warning restore 0414
        ...
        [SerializeField] private string processTemplate = "{0}%";
        ...
        private IDisposable _disposeUpdatePerFrame;
        ...
        private bool _pause;
        private float _deltaTimeUnscale = 0.02f;
        ...
        private void Awake()
        {
            _deltaTimeUnscale = Time.unscaledDeltaTime;
        ...
        private void Start()
        {
            ...
            _disposeUpdatePerFrame = Observable.EveryUpdate()
                .Subscribe(_ =>
                {
                    UpdatePerFrame();
                })
        ...
        private void UpdateUi()
        {
            if (loadingType == ELoadingType.Async)
            {
                ...
                _lerpValue = Mathf.Lerp(_lerpValue, p, _deltaTimeUnscale * value);
            }
        ...
        private void SetupUi()
        {
            ...
            if (isProcessBar)
            {
                if (processBar != null) processBar.value = 0;
                if (canvasGroupProcessBar != null)
                {
        ...
        private IEnumerator StartFakeLoading()
        {
            ...
            while (_lerpValue < 1)
            {
                if (!_pause)
                {
                    _lerpValue += _deltaTimeUnscale / value;
                }
                else
                {
                    if (_lerpValue < 0.32f)
                    {
                        _lerpValue += _deltaTimeUnscale / value / 5f;
                    }
                    else if (_lerpValue < 0.5f)
                    {
                        _lerpValue += _deltaTimeUnscale / value / 12f;
                    }
                    else if (_lerpValue < 0.8f)
                    {
                        _lerpValue += _deltaTimeUnscale / value / 24f;
                    }
                    else if (_lerpValue < 0.95f)
                    {
                        _lerpValue += _deltaTimeUnscale / value / 30f;
                    }
                }
        ...
        private IEnumerator TipsLoop()
        {
            ...
            if (_isTipFadeOut)
            {
                ...
                while (alpha.a < 1)
                {
                    alpha.a += _deltaTimeUnscale * tipFadeSpeed;
                    tipText.color = alpha;
                    yield return null;
                }
        ...
        private IEnumerator LoadNextSceneIe()
        {
            ...
            fadeImageCanvas.alpha = 1;
            if (isProcessBar && canvasGroupProcessBar != null)
            {
